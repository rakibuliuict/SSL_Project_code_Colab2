...
# ------------------ Loss and Optimizer ------------------ #
loss_fn = DiceFocalLoss(sigmoid=True, to_onehot_y=False, softmax=False)
optimizer = optim.Adam(net.parameters(), lr=args.lr)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for batch in tqdm(dataloader, ncols=80):
        ...
        # Fix shape if needed (H, W, D → D, H, W)
        if images.shape[2:] == (160, 160, 16):
            images = images.permute(0, 1, 4, 2, 3)
            labels = labels.permute(0, 1, 4, 2, 3)

        outputs = net(images)
        labels = labels.float()  # required for DiceFocalLoss
    ...
    if (epoch + 1) % 5 == 0:
        net.eval()
        dice_score = test_3d_patch.var_all_case_LA(
            net, num_classes=1, patch_size=(160, 160, 16), stride_xy=18, stride_z=4
        )
        logging.info(f"Epoch [{epoch+1}/{args.epochs}], Loss: {avg_loss:.4f}, Dice: {dice_score:.4f}")
